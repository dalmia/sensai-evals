def create_selected_run_view():
    """Create the selected run view component with all HTML and JavaScript"""
    return """
    <script>
        ...
        window.createSelectedRunViewHTML = function(selectedRun, metadataSidebarOpen, annotationSidebarOpen) {
            ...
            // Check if generateMessagesHTML is available, if not provide fallback
            if (typeof window.generateMessagesHTML === 'function') {
                messagesHtml = window.generateMessagesHTML(messages, context, questionType);
            } else {
                // Fallback: simple message rendering
                console.warn('generateMessagesHTML not available, using fallback');
